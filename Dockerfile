...
RUN sudo apt-get -y install apt-fast
RUN sudo apt-fast -y upgrade
...
RUN sudo add-apt-repository -y ppa:apt-fast/stable
RUN sudo add-apt-repository -y ppa:graphics-drivers/ppa

RUN sudo apt-fast install -y ubuntu-drivers-common libvorbis-dev libflac-dev libsndfile-dev cmake \
         ...
         parallel exuberant-ctags vim-nox python-powerline python3-pip
RUN sudo apt-fast install -y tigervnc-standalone-server firefox lsyncd mesa-common-dev ack

# cat << 'EOF' >> /appuser/.ssh/config
# Host *
